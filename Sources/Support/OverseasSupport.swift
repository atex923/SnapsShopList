...
struct OverseasTextCaptureButton: View {
    ...
    var body: some View {
        Menu {
            Button("使用相機擷取", systemImage: "camera.fill") {
                isCameraPresented = true
            }
            PhotosPicker(selection: $selectedLibraryItem, matching: .images) {
                Label("從相簿擷取", systemImage: "photo.on.rectangle")
            }
        } label: {
            ...
            Task {
                ...
                do {
                    guard let data = try await item.loadTransferable(type: Data.self) else {
                        errorMessage = "無法讀取選取的照片。"
                        return
                    }
                    recognizeAfterCameraDismisses(data)
                } catch {
                    AppErrorLogger.record(error, category: "海外 OCR", context: "相簿：\(title)")
                    errorMessage = error.localizedDescription
                }
            }
        }
        .fullScreenCover(isPresented: $isCameraPresented) {
            CameraPhotoPicker(onPhotoData: { data, _ in
