...
struct ProductDetailView: View {
    ...
    private func image(at index: Int) -> UIImage? {
        ...
        return ProductPhotoStore.load(fileName: photos[index].fileName)
    }
    ...
    private func addPhoto(_ image: UIImage) {
        ...
        do {
            ...
            let photo = ProductPhoto(fileName: fileName, product: product)
            ...
            ScrollView {
                LazyVStack(spacing: 18) {
                    ForEach(product.sortedPhotos) { photo in
                        if let image = ProductPhotoStore.load(fileName: photo.fileName) {
