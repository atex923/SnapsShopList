...
struct HomeView: View {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if let product = scannedProduct {
                        ...
                        if let latest = product.sortedRecords.first {
                            HStack(alignment: .firstTextBaseline) {
                                Text(latest.effectiveUnitPrice.formatted(.currency(code: "TWD")))
                                    .font(.title2.bold())
    ...
    private func priceChangeLabel(for product: Product) -> some View {
        let records = product.sortedRecords
        if records.count >= 2 {
            ...
            if abs(difference) < 0.005 {
                ...
                Label(
                    abs(difference).formatted(.currency(code: "TWD")),
    ...
    private var resultTint: Color {
        guard let records = scannedProduct?.sortedRecords, records.count >= 2 else {
...
private struct ShoppingListCompactRow: View {
    ...
    private var compactRow: some View {
        HStack(spacing: 8) {
            ...
            barcodeButton(compact: true)
            deleteButton
        }
    }

    private func barcodeButton(compact: Bool) -> some View {
        Button {
