...
final class Product {
    ...
    var priceStatistics: ProductPriceStatistics? {
        guard let currencyCode = latestRecord?.normalizedCurrencyCode else { return nil }
        return ProductPriceStatistics(records: records ?? [], currencyCode: currencyCode)
    }

    func priceStatistics(currencyCode: String) -> ProductPriceStatistics? {
        ProductPriceStatistics(records: records ?? [], currencyCode: currencyCode)
    }
...
struct ProductPriceStatistics {
    ...
    init?(records: [PurchaseRecord], currencyCode: String) {
        ...
        let validRecords = records.filter {
            $0.price >= 0 && $0.price.isFinite && $0.normalizedCurrencyCode == normalizedCurrencyCode
        }
        guard
