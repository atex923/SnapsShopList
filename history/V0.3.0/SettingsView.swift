...
struct SettingsView: View {
    ...
    private var purchasePreferencesSection: some View {
        Section("採買設定") {
            Picker("預設貨幣", selection: $defaultCurrencyCode) {
                ForEach(SupportedCurrency.allCases) { currency in
                    Text(currency.displayName).tag(currency.rawValue)
                }
            }

            Text("切換後，新建立的採買紀錄會使用此貨幣；既有紀錄仍保留原本貨幣。每筆資料也可在新增或編輯時個別調整。")
                .font(.caption)
