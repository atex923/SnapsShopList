...
struct SettingsView: View {
    ...
    private var overseasModeSection: some View {
        Section {
            ...
            Text(overseasModeEnabled
                 ? "新採買會持續使用所選貨幣，直到再次切換。OCR 只在按下文字擷取時啟動；網路查詢前會先顯示送出的文字。"
                 : "國內模式以快速記錄為主，新採買使用台幣，不載入 OCR 或外文名稱查詢功能。切換模式不會改寫既有資料。")
        }
    }
    ...
    private var backupSection: some View {
        Section("備份與匯入") {
            ...
            Text("系統檔案畫面可選擇 iCloud Drive、Google Drive、我的 iPhone或其他檔案服務。完整備份包含商品、採買紀錄、店家、待買清單、食材庫歷史及照片。")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
