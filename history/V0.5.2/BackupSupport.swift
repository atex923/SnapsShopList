...
enum BackupManager {
    ...
    nonisolated static func validate(_ archive: SnapsBackupArchive) throws {
        guard (1...3).contains(archive.schemaVersion) else {
    ...
    static func importArchive(
        ...
        var pantryIDs = mode == .replace
            ? Set<UUID>()
            : Set(try context.fetch(FetchDescriptor<PantryEntry>()).map(\.id))
        let productsByID = Dictionary(
            productsByBarcode.values.map { ($0.id, $0) },
            uniquingKeysWith: { first, _ in first }
        )
        for item in archive.pantryEntries ?? [] where pantryIDs.insert(item.id).inserted {
