    ...
    private var pantrySection: some View {
        NavigationLink {
            ...
            HStack(spacing: 12) {
                ...
                Spacer()
                if !scannedBarcode.isEmpty {
                    if scannedProduct?.currentPantryEntry == nil {
                        Image(systemName: "exclamationmark.triangle.fill")
                            .foregroundStyle(.orange)
                            .accessibilityLabel("掃描商品未在食材庫")
                    } else {
                        Text("在庫")
                            .font(.subheadline.bold())
        ...
        NavigationLink {
            ...
            HStack(spacing: 12) {
                Label("海外購物數量庫", systemImage: "globe")
                    .font(.title3.bold())
                Spacer()
                if let product = scannedProduct, !product.sortedRecords.isEmpty {
                    Text("\(product.sortedRecords.count) 筆")
                        .font(.caption.bold())
