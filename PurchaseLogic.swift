...
enum PurchaseRecordStatus: String, Codable {
    case quickDraft
    case comparison
    case wishlistQuote
    case complete
...
enum PurchaseLocationSource: String, Codable {
    case liveGPS
    case photoMetadata
    case manual
    case none
...
enum PurchaseCurrencyPolicy {
    static func activeCode(overseasModeEnabled: Bool, preferredCode: String) -> String {
        overseasModeEnabled
            ? SupportedCurrency.normalized(preferredCode).rawValue
            : SupportedCurrency.TWD.rawValue
    }
}
...
struct PurchaseDraft: Equatable {
    ...
    var isGroupPackage = false
    var groupContentCountText = "1"
}
...
enum PurchaseValidator {
    static func validate(_ draft: PurchaseDraft, requiresProductName: Bool) -> ValidatedPurchase? {
        ...
        guard
            ...
            let groupContentCount = validatedGroupContentCount(draft)
        else { return nil }
    ...
    private static func validatedGroupContentCount(_ draft: PurchaseDraft) -> Int? {
        guard draft.isGroupPackage else { return 1 }
        return LocalizedNumberParser.positiveInteger(from: draft.groupContentCountText)
    }
}
...
enum PriceCalculator {
    static func effectiveUnitPrice(
        totalPrice: Double,
        purchaseQuantity: Int,
        isBuyOneGetOne: Bool,
        groupContentCount: Int = 1
    ) -> Double {
        let purchasedUnits = Double(max(1, purchaseQuantity) * max(1, groupContentCount))
    ...
    static func referenceUnitPrice(
        totalPrice: Double,
        purchaseQuantity: Int,
        isBuyOneGetOne: Bool,
        groupContentCount: Int = 1,
        ...
        guard let normalizedAmount = normalizedAmount(amount: amount, unit: unit), normalizedAmount > 0,
              let normalizedUnit = normalizedUnit(unit) else { return nil }
        let referenceAmount = normalizedUnit == "ml" && normalizedAmount > 1_000 ? 1_000.0 : 100.0
        let label = referenceAmount == 1_000 ? "每 1 L" : "每 100 \(normalizedUnit)"
        let value = effectiveUnitPrice(
            totalPrice: totalPrice,
            purchaseQuantity: purchaseQuantity,
            isBuyOneGetOne: isBuyOneGetOne,
            groupContentCount: groupContentCount
        ) / normalizedAmount * referenceAmount
        return (value, label)
    }
}
